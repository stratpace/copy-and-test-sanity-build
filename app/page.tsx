...
export const metadata: Metadata = {
  title: 'Home | Stratpace Advisory',
  description: 'Clarity in a Complex Market. Data-driven strategic advisory for the world\'s leading enterprises.',
};

export default function Home() {
  // In a real Sanity implementation, this would be:
  // const reportsData = await client.fetch(groq`*[_type == "insight"]...`)
            ...
            {displayedServices.map((service, index) => (
              <ScrollReveal key={service.id} delay={index * 100}>
                <div className="group flex flex-col gap-4 p-6 border border-border hover:border-gold-600/50 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 bg-background rounded-sm h-full">
                  ...
                  <Link href="/capabilities" className="mt-auto pt-4 text-sm font-bold text-navy-900 flex items-center gap-1 group-hover:gap-2 transition-all">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 border-t border-l border-border">
            {displayedIndustries.map((industry, index) => (
               <ScrollReveal key={industry.id} delay={index * 50}>
                <Link href={`/expertise/${industry.id}`} className="group relative block p-8 border-r border-b border-border hover:bg-white transition-colors h-full">
        ...
        <div className="max-w-[1280px] mx-auto px-6">
          <ScrollReveal>
            <div className="flex justify-between items-center mb-12">
              ...
              <Link href="/reports" className="text-sm font-semibold text-navy-900 border-b border-navy-900 pb-1 hover:text-gold-600 hover:border-gold-600 transition-colors">View all reports</Link>
            </div>
          </ScrollReveal>
            ...
            <div className="lg:col-span-5 flex flex-col gap-10 lg:pl-6 lg:border-l border-border">
              {sideReports.map((article, index) => (
                <ScrollReveal key={article.id} delay={200 + index * 100}>
                  <Link href="/reports" className="flex flex-col gap-3 group">
